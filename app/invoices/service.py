...
def set_exportoption(r, typ, rpt_name):
    ...
    if typ in "csv":
        ...
        exp.DiskFileName = 'app/static/invoice_output/' + key + '.csv'
        exp.FormatType = 5
        filename = key + '.csv'
    elif typ in "pdf":
        exp.DestinationType = 1
        exp.FormatType = 31
        exp.DiskFileName = 'app/static/invoice_output/' + key + '.pdf'
        filename = key + '.pdf'
    elif typ in "xls":
        exp.DestinationType = 1
        exp.FormatType = 36
        exp.DiskFileName = 'app/static/invoice_output/' + key + '.xls'
        filename = key + '.xls'
    # elif typ in "html":
    #     exp.DestinationType = 1
    #     exp.FormatType = 24
