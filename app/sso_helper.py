...
def check_unit_privilege_on_read_db(orm_execute_state, member_of_list, modelName):
    ...
    if current_user['member_of_list'] and not current_user['role_name'] == "developer":
        if (
    ...
    if orm_execute_state.is_select:
        ...
        if col_descriptions[0]['entity'] is model or col_descriptions[0]['name'] == 'count':
            ...
            if current_user:
                ...
                isDeveloper = True if current_user['role_name'] == "developer" else None
                if member_of_list and not isDeveloper:
                    if (
                            orm_execute_state.is_select and
                ...
                if id_employee and not isDeveloper:
...
def check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list):
    ...
    if 'id_unit' in mapper.columns:
        isDeveloper = True if current_user['role_name'] == "developer" else None
        if member_of_list and target.id_unit and not isDeveloper:
...
def getAuth(url, headers, jsonPayload):
    try:
        req = requests.post(url, headers=headers, json=jsonPayload)
        req.raise_for_status()  # Raise an exception for HTTP error responses
        user_data = req.json()
        return user_data
    except requests.exceptions.RequestException as e:
        logger.error(f"Error fetching authentication: {e}")
        return None

def verify_token(authorizationHeader, request, url):
    try:
        headers = {"Authorization": authorizationHeader}
        jsonPayload = {
            'x_endpoint': request.path,
            'x_method': request.method,
            'x_origin': request.origin or os.environ.get('DOMAIN')
        }
        user_data = getAuth(url, headers, jsonPayload)
        # print(user_data)
        ...
        logger.error(f'Verify token to SSO ({url}) FAILED: {e}')
        raise
...
def token_required(fn):
    ...
    def decorator(*args, **kwargs):
        ...
        if 'Bearer ' not in authorizationHeader:
            ...
            response.status_code = 401
            return response
        user_data = verify_token(authorizationHeader, request, url)
...
def domain_claims():
    ...
    if data := req.json():
        ...
        os.environ['APPDESC'] = data['data']['app']['info']['description'] or ""
        os.environ['APP_LOGO'] = data['data']['app']['logo']['appLogo'] or ""
...
def syncronize_resource():
    ...
    @token_required
    def get(self):
        tahun_anggaran = current_user['data_year']
        print(tahun_anggaran)
        # return GeneralGetList(doc, crudTitle)
#
