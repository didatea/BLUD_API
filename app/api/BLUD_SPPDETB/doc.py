...
from . import apiName, apiPath, crudTitle, docName, respAnSPPDETByloadFields, enableSPPDETBgination
...
class doc:
    ...
    default_data_response = api.model(
        f"{docName}_object", respAnSPPDETByloadFields
    )
    ...
    getRespDoc = generateDefaultDoc(api, default_data_response, crudTitle, "get-list-pagination" if enableSPPDETBgination == True else "get-list", docName)
