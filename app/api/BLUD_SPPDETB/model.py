...
class SPPDETB(db.Model):
    __tablename__ = 'SPPDETB'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    IDREK = db.Column(db.BigInteger, db.ForeignKey("DAFTREKENING.id"), nullable=False)
    IDSPP = db.Column(db.BigInteger, db.ForeignKey("SPP.id"), nullable=False)
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, SPPDETB)


@event.listens_for(SPPDETB, 'before_insert')
...
@event.listens_for(SPPDETB, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(SPPDETB, 'before_delete')
...
@event.listens_for(SPPDETB, 'after_insert')
