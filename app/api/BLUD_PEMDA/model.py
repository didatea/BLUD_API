...
class PEMDA(db.Model):
    __tablename__ = 'PEMDA'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    CONFIGID = db.Column(db.String(2), nullable=False)
    CONFIGVAL = db.Column(db.BigInteger, nullable=False)
    CONFIGDES = db.Column(db.String(100), nullable=True)


# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, PEMDA)


@event.listens_for(PEMDA, 'before_insert')
...
@event.listens_for(PEMDA, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(PEMDA, 'before_delete')
...
@event.listens_for(PEMDA, 'after_insert')
