...
class PAGUSKPD(db.Model):
    __tablename__ = 'PAGUSKPD'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    IDUNIT = db.Column(db.BigInteger, nullable=False)
    KDTAHAP = db.Column(db.Integer, nullable=False)
    NILAIUP = db.Column(mssql.MONEY, nullable=False)
    NILAI = db.Column(mssql.MONEY, nullable=False)
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, PAGUSKPD)


@event.listens_for(PAGUSKPD, 'before_insert')
...
@event.listens_for(PAGUSKPD, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(PAGUSKPD, 'before_delete')
...
@event.listens_for(PAGUSKPD, 'after_insert')
