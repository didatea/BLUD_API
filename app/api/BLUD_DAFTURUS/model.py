...
class DAFTURUS(db.Model):
    __tablename__ = 'DAFTURUS'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    KDURUS = db.Column(db.String(50), nullable=False)
    NMURUS = db.Column(db.String(500), nullable=False)
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, DAFTURUS)


@event.listens_for(DAFTURUS, 'before_insert')
...
@event.listens_for(DAFTURUS, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(DAFTURUS, 'before_delete')
...
@event.listens_for(DAFTURUS, 'after_insert')
