...
class JBAYAR(db.Model):
    __tablename__ = 'JBAYAR'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    KDBAYAR = db.Column(db.Integer, nullable=True)
    URAIANBAYAR = db.Column(db.String(500), nullable=False)
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, JBAYAR)


@event.listens_for(JBAYAR, 'before_insert')
...
@event.listens_for(JBAYAR, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(JBAYAR, 'before_delete')
...
@event.listens_for(JBAYAR, 'after_insert')
