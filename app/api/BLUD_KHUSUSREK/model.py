...
class KHUSUSREK(db.Model):
    __tablename__ = 'KHUSUSREK'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    KDKHUSUS = db.Column(db.Integer, nullable=False)
    NMKHUSUS = db.Column(db.String(512), nullable=True)


# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, KHUSUSREK)


@event.listens_for(KHUSUSREK, 'before_insert')
...
@event.listens_for(KHUSUSREK, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(KHUSUSREK, 'before_delete')
...
@event.listens_for(KHUSUSREK, 'after_insert')
