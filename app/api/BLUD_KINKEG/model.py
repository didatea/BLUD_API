...
class KINKEG(db.Model):
    __tablename__ = 'KINKEG'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    KDJKK = db.Column(db.BigInteger, nullable=False)
    ...
    TOLOKUR = db.Column(db.String(4096), nullable=True)
    TARGET = db.Column(db.String(4096), nullable=True)
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, KINKEG)


@event.listens_for(KINKEG, 'before_insert')
...
@event.listens_for(KINKEG, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(KINKEG, 'before_delete')
...
@event.listens_for(KINKEG, 'after_insert')
...
@event.listens_for(KINKEG, 'after_update')
...
@event.listens_for(KINKEG, 'after_delete')
