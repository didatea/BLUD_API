...
class APPSETT(db.Model):
    __tablename__ = 'APPSETT'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    KDSET = db.Column(db.String(2), nullable=False)
    VALSET = db.Column(db.String(100), nullable=False)
    VALDESC = db.Column(db.String(100), nullable=True)
    MODEENTRY = db.Column(db.BigInteger, nullable=False)
    VALLIST = db.Column(db.String(100), nullable=False)


# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, APPSETT)


@event.listens_for(APPSETT, 'before_insert')
...
@event.listens_for(APPSETT, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(APPSETT, 'before_delete')
...
@event.listens_for(APPSETT, 'after_insert')
...
@event.listens_for(APPSETT, 'after_update')
...
@event.listens_for(APPSETT, 'after_delete')
