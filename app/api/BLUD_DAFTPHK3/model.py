...
class DAFTPHK3(db.Model):
    __tablename__ = 'DAFTPHK3'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    NMPHK3 = db.Column(db.String(100), nullable=False)
    NMINST = db.Column(db.String(100), nullable=False)
    IDBANK = db.Column(db.BigInteger, db.ForeignKey("JBANK.id"), nullable=True)
    CABANGBANK = db.Column(db.String(200), nullable=False)
    ALAMATBANK = db.Column(db.String(200), nullable=True)
    NOREKBANK = db.Column(db.String(50), nullable=False)
    IDJUSAHA = db.Column(db.BigInteger, nullable=True)
    ALAMAT = db.Column(db.String(100), nullable=True)
    TELEPON = db.Column(db.String(100), nullable=True)
    NPWP = db.Column(db.String(50), nullable=False)
    WARGANEGARA = db.Column(db.String(100), nullable=True)
    STPENDUDUK = db.Column(db.String(100), nullable=True)
    STVALID = db.Column(db.Integer, nullable=True)
    ...
    @property
    def NMBANK(self):
        return f" {self.JBANK.NMBANK}" if self.JBANK else None

# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, DAFTPHK3)


@event.listens_for(DAFTPHK3, 'before_insert')
...
@event.listens_for(DAFTPHK3, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(DAFTPHK3, 'before_delete')
...
@event.listens_for(DAFTPHK3, 'after_insert')
...
@event.listens_for(DAFTPHK3, 'after_update')
...
@event.listens_for(DAFTPHK3, 'after_delete')
