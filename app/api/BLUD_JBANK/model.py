...
class JBANK(db.Model):
    __tablename__ = 'JBANK'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    KDBANK = db.Column(db.String(10), nullable=False)
    NMBANK = db.Column(db.String(500), nullable=False)
    URAIAN = db.Column(db.String(500), nullable=False)
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, JBANK)


@event.listens_for(JBANK, 'before_insert')
...
@event.listens_for(JBANK, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(JBANK, 'before_delete')
...
@event.listens_for(JBANK, 'after_insert')
