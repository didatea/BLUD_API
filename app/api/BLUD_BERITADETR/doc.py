...
from . import apiName, apiPath, crudTitle, docName, respAnBERITADETRyloadFields, enableBERITADETRgination
...
class doc:
    ...
    default_data_response = api.model(
        f"{docName}_object", respAnBERITADETRyloadFields
    )
    ...
    getRespDoc = generateDefaultDoc(api, default_data_response, crudTitle, "get-list-pagination" if enableBERITADETRgination == True else "get-list", docName)
