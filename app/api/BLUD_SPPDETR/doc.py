...
from . import apiName, apiPath, crudTitle, docName, respAnSPPDETRyloadFields, enableSPPDETRgination
...
class doc:
    ...
    default_data_response = api.model(
        f"{docName}_object", respAnSPPDETRyloadFields
    )
    ...
    getRespDoc = generateDefaultDoc(api, default_data_response, crudTitle, "get-list-pagination" if enableSPPDETRgination == True else "get-list", docName)
