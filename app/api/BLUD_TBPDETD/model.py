...
class TBPDETD(db.Model):
    __tablename__ = 'TBPDETD'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    IDUNIT = db.Column(db.BigInteger, db.ForeignKey("DAFTUNIT.id"), nullable=False)
    IDTBP = db.Column(db.BigInteger, db.ForeignKey("TBP.id"), nullable=False)
    IDREK = db.Column(db.String(5), db.ForeignKey("DAFTREKENING.id"), nullable=False)
    ...
    # TBPDETDD = db.relationship('TBPDETDD', backref=db.backref(f'{modelName}'), lazy="dynamic")
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, TBPDETD)


@event.listens_for(TBPDETD, 'before_insert')
...
@event.listens_for(TBPDETD, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(TBPDETD, 'before_delete')
...
@event.listens_for(TBPDETD, 'after_insert')
