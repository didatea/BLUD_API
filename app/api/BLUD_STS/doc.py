...
from . import apiName, apiPath, crudTitle, docName, respAnSTSyloadFields, enableSTSgination
...
class doc:
    ...
    default_data_response = api.model(
        f"{docName}_object", respAnSTSyloadFields
    )
    ...
    getRespDoc = generateDefaultDoc(api, default_data_response, crudTitle, "get-list-pagination" if enableSTSgination == True else "get-list", docName)
