...
class BPKDETR(db.Model):
    __tablename__ = 'BPKDETR'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    IDBPK = db.Column(db.BigInteger, nullable=False)
    ...
    IDNOJETRA = db.Column(db.Integer, default=21, nullable=True)
...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, BPKDETR)


@event.listens_for(BPKDETR, 'before_insert')
...
@event.listens_for(BPKDETR, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(BPKDETR, 'before_delete')
...
@event.listens_for(BPKDETR, 'after_insert')
