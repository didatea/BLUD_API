...
class MPGRM(db.Model):
    __tablename__ = 'MPGRM'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    IDURUS = db.Column(db.BigInteger, nullable=False)
    NMPRGRM = db.Column(db.String(512), nullable=False)
    NUPRGRM = db.Column(db.String(5), nullable=False)
    IDPRIODA = db.Column(db.Integer, nullable=True)
    IDPRIOPROV = db.Column(db.Integer, nullable=True)
    IDPRIONAS = db.Column(db.Integer, nullable=True)
    ...
    PGRMUNIT = db.relationship("PGRMUNIT", backref=modelName, lazy="dynamic")


# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, MPGRM)


@event.listens_for(MPGRM, 'before_insert')
...
@event.listens_for(MPGRM, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(MPGRM, 'before_delete')
...
@event.listens_for(MPGRM, 'after_insert')
