...
from . import apiName, apiPath, crudTitle, docName, respAnSPPDETPyloadFields, enableSPPDETPgination
...
class doc:
    ...
    default_data_response = api.model(
        f"{docName}_object", respAnSPPDETPyloadFields
    )
    ...
    getRespDoc = generateDefaultDoc(api, default_data_response, crudTitle, "get-list-pagination" if enableSPPDETPgination == True else "get-list", docName)
