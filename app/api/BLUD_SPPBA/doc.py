...
from . import apiName, apiPath, crudTitle, docName, respAnSPPBAyloadFields, enableSPPBAgination
...
class doc:
    ...
    default_data_response = api.model(
        f"{docName}_object", respAnSPPBAyloadFields
    )
    ...
    getRespDoc = generateDefaultDoc(api, default_data_response, crudTitle, "get-list-pagination" if enableSPPBAgination == True else "get-list", docName)
