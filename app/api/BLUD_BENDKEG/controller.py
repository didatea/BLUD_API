...
class List(Resource):
    ...
    def get(self):
        ...
        args["IDBEND"]
        sqlQuery = text(f'''
            SELECT p.IDUNIT, p.NAMA, m.NUKEG, m.NMKEGUNIT, b.IDUNIT, b.IDBEND, b.IDKEG
            FROM BENDKEG AS b INNER JOIN MKEGIATAN AS m ON m.id = b.IDKEG 
            INNER JOIN BEND AS b2 ON b2.id = b.IDBEND
            INNER JOIN PEGAWAI AS p ON p.id = b2.IDPEG
            WHERE IDBEND={args["IDBEND"]}''')
