...
class JNSPAJAK(db.Model):
    __tablename__ = 'JNSPAJAK'
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    NMJNSPAJAK = db.Column(db.String(50), nullable=True)

    PAJAK = db.relationship('PAJAK', backref=db.backref(f'{modelName}'), lazy="dynamic")

# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, JNSPAJAK)


@event.listens_for(JNSPAJAK, 'before_insert')
...
@event.listens_for(JNSPAJAK, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(JNSPAJAK, 'before_delete')
...
@event.listens_for(JNSPAJAK, 'after_insert')
...
@event.listens_for(JNSPAJAK, 'after_update')
...
@event.listens_for(JNSPAJAK, 'after_delete')
