...
class GOLONGAN(db.Model):
    ...
    NMGOL = db.Column(db.String(50), nullable=True)
    PANGKAT = db.Column(db.String(30), nullable=True)

    @property
    def KDGOL_STRIP(self):
        return self.KDGOL.rstrip()


# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, GOLONGAN)


@event.listens_for(GOLONGAN, 'before_insert')
...
@event.listens_for(GOLONGAN, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(GOLONGAN, 'before_delete')
...
@event.listens_for(GOLONGAN, 'after_insert')
