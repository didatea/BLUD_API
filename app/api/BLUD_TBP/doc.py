...
from . import apiName, apiPath, crudTitle, docName, respAnTBPyloadFields, enableTBPgination
...
class doc:
    ...
    default_data_response = api.model(
        f"{docName}_object", respAnTBPyloadFields
    )
    ...
    getRespDoc = generateDefaultDoc(api, default_data_response, crudTitle, "get-list-pagination" if enableTBPgination == True else "get-list", docName)
