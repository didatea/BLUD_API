...
class PAJAK(db.Model):
    __tablename__ = 'PAJAK'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    KDPAJAK = db.Column(db.String(10), nullable=False)
    NMPAJAK = db.Column(db.String(512), nullable=True)
    URAIAN = db.Column(db.String(512), nullable=True)
    RUMUSPAJAK = db.Column(db.String(36), nullable=True)
    IDJNSPAJAK = db.Column(db.Integer, db.ForeignKey("JNSPAJAK.id"), nullable=True)
    ...
    @property
    def NMJNSPAJAK(self):
        return f" {self.JNSPAJAK.NMJNSPAJAK}" if self.JNSPAJAK else None

# BEFORE TRANSACTION: CHECK PRIVILEGE UNIT
@event.listens_for(db.session, "do_orm_execute")
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, PAJAK)


@event.listens_for(PAJAK, 'before_insert')
...
@event.listens_for(PAJAK, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(PAJAK, 'before_delete')
...
@event.listens_for(PAJAK, 'after_insert')
...
@event.listens_for(PAJAK, 'after_update')
...
@event.listens_for(PAJAK, 'after_delete')
